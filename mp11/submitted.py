...
class q_learner():
    ...
    def choose_unexplored_action(self, state):
        ...
        '''
        # raise RuntimeError('You need to write this!')
        underexplored_actions = np.where(self.N[tuple(state)] < self.nfirst)[0]
        if underexplored_actions.size > 0:
            action = random.choice(underexplored_actions) - 1
    ...
    def q_local(self, reward, newstate):
        ...
        '''
        # raise RuntimeError('You need to write this!')
        return reward + self.gamma * np.max(self.Q[tuple(newstate)])
...
class deep_q():
    ...
    def learn(self, state, action, reward, newstate):
        ...
        # raise RuntimeError('You need to write this!')
        state_tensor = torch.tensor(state, dtype=torch.float).unsqueeze(0)
        newstate_tensor = torch.tensor(newstate, dtype=torch.float).unsqueeze(0)
        q_values = self.model(state_tensor)
